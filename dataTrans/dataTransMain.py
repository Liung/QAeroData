...
class DataTransWidget(QDialog, Ui_dataTransWidget):
    ...
    def initBalance(self):
        ...
        self.balance.setAngleStartCol(self.spbAngleStartCol.value() - 1)
        self.balance.setAngleEndCol(self.spbAngleEndCol.value() - 1)
        self.balance.setForceStartCol(self.spbForceStartCol.value() - 1)
        self.balance.setForceEndCol(self.spbForceEndCol.value() - 1)
    ...
    def on_pbtnGenerateFile_clicked(self):
        ...
        try:
            if self.tabWidget.currentIndex() == 0:      # single file translation
                ...
                QMessageBox.about(self, u"{0}".format(__AppName__), u"文件转换完成。\n")
    ...
    def on_spbAngleStartCol_valueChanged(self, col):
        self.balance.setAngleStartCol(col - 1)

    @pyqtSignature("int")
    def on_spbAngleEndCol_valueChanged(self, col):
        self.balance.setAngleEndCol(col - 1)

    @pyqtSignature("int")
    def on_spbForceStartCol_valueChanged(self, col):
        self.balance.setForceStartCol(col - 1)
