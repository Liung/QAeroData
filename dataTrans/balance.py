...
class Balance(object):
    ...
    def __init__(self, staFile=u"", dynFile=u"", bodyFile=u"", aeroFile=u"",
                 headerRows=1, footerRows=0, angleStartCol=0, angleEndCol=3,
                 forceStartCol=4, forceEndCol=9, aircraftModel=None, balanceSty=-1,
                 balanceCoes=None):
        """
        ...
        :param headerRows:
        :param footerRows:
        :param angleStartCol: 角度起始列是从0开始算起（以python中从0计数），所以实际角度所在列需要减去1
        :param angleEndCol:
        :param forceStartCol: 使用同angleStartCol
        :param forceEndCol:
        :param aircraftModel:
        :param balanceSty:
        :param balanceCoes:
        ...
        self._angleCols = angleEndCol - angleStartCol + 1
...
'''.format(u'静态文件:', self._staFile,
    ...
    def translateData(self):
        sf = self._staFile  # static file's name
        df = self._dynFile  # dynamic file's name
        bf = self._bodyFile  # body file's name
        af = self._aeroFile  # aero file's name

        hr = self._headerRows  # data file's header nums
        fr = self._footerRows  # data file's footer nums
        asc = self._angleStartCol  # angle start column
        aec = self._angleEndCol  # angle end column
        ac = aec - asc + 1  # angle columns
        fsc = self._forceStartCol  # force and moment start column
        fec = self._forceEndCol  # force and moment end column
        fc = fec - fsc + 1  # force and moment columns
        try:
            # load static file and dynamic file
            staData = genfromtxt(fname=sf, skip_header=hr, skip_footer=fr)
            dynData = genfromtxt(fname=df, skip_header=hr, skip_footer=fr)
            if self._columnOffset:
                for col in range(asc, aec + 1):
                    offset = self._columnOffset.get(col+1, 0)   # col+1是因为偏移量字典存储的是用户输入的量，即以1开始计数的序列
                    staData[:, col] = staData[:, col] + offset
                    dynData[:, col] = dynData[:, col] + offset
            staAngle, staForce = (staData[:, asc:(aec+1)],
                                  staData[:, fsc:(fec+1)])
            dynAngle, dynForce = (dynData[:, asc:(aec+1)],
                                  dynData[:, fsc:(fec+1)])
            ...
            time = (np.arange(ang.shape[0]) + 1)*0.001
            time = time.reshape((-1, 1))
            Mb = hstack((hstack((time, ang)), Cb))
            Ma = hstack((hstack((time, ang)), Ca))

            aero_header = ['time', 'alpha', 'beta', 'theta', 'gamma',
                           'CD', 'CL', 'CZ', 'Cx', 'Cy', 'Cz']
            body_header = ['time', 'theta', 'phi', 'psi', 'gamma',
                           'CA', 'CN', 'CY', 'Cl', 'Cn', 'Cm']
            savetxt(bf, Mb, fmt='%-15.8f',
                    header='\t'.join(body_header),
                    comments='')
            savetxt(af, Ma, fmt='%-15.8f',
                    header='\t'.join(aero_header),
                    comments='')
            return True, None
        except Exception, e:
...
if __name__ == '__main__':
    ...
    saccon = AircraftModel(area=0.0521, span=0.4, rootChord=0.2759,
                           refChord=0.1246, speed=25.)
    coes = np.loadtxt("G18.txt")

    file_dir = r"D:\LiuChao\ExpData\SACCON\Pitch\export"
    save_dir = r"D:\LiuChao\ExpData\SACCON\Pitch\data"

    dyn_file_list = [fn for fn in os.listdir(file_dir)
                     if fn.upper().endswith("DFE.DAT")]
    sta_file_list = [fn for fn in os.listdir(file_dir)
                     if fn.upper().endswith("JFE.DAT")]

    if len(dyn_file_list) != len(sta_file_list):
        raise Exception("Error")

    for staFile, dynFile in zip(sta_file_list, dyn_file_list):

        staFilePath = os.path.join(file_dir, staFile)
        dynFilePath = os.path.join(file_dir, dynFile)

        bodyFile = dynFile.split('DFE')[0] + 'FET.dat'
        aeroFile = dynFile.split('DFE')[0] + 'FEQ.dat'
        bodyFilePath = os.path.join(save_dir, bodyFile)
        aeroFilePath = os.path.join(save_dir, aeroFile)
        ...
        b = Balance(staFile=staFilePath, dynFile=dynFilePath,
                    bodyFile=bodyFilePath, aeroFile=aeroFilePath,
                    aircraftModel=saccon, headerRows=1,
                    angleStartCol=1, angleEndCol=4,
                    forceStartCol=5, forceEndCol=10,
                    balanceSty=2, balanceCoes=coes)

        state, msg = b.translateData()
        if not state:
            print msg

    # data = np.loadtxt(r"D:/LiuChao/Simulation/Test_DR_P30R00AR30_N04FT.txt", skiprows=0)
    # alpha = data[:, 2]
    # CN = data[:, 7]
    # plt.plot(alpha, CN, "k", lw=3)
    #
    # print u"先平均在处理", data.shape
    #
    # data1 = np.loadtxt(r"D:/LiuChao/Simulation/Test_DR_P30R00AR30PD08_N04FT.txt", skiprows=0)
    # alpha1 = data1[:, 2]
    # CN1 = data1[:, 7]
    # plt.plot(alpha1, CN1, 'r')
    #
    # print u"多周期：", data1.shape
    #
    # peroid = 8
    # m, n = data1.shape
    # average_data1 = np.zeros((m/peroid, n))
    # for i in range(n):
    #     average_data1[:, i] = data1[:, i].reshape((peroid, -1)).sum(axis=0)/peroid
    #
    # alpha2 = average_data1[:, 2]
    # CN2 = average_data1[:, 7]
    # plt.plot(alpha2, CN2, 'g', lw=3)
    #
    # print u"先处理，再平均：", average_data1
    #
    # np.allclose(data, average_data1)
    #
    # plt.show()