...
def images2gif(images, giffile, durations=0.05, loops=1):
    ...
    msg = str(frames) + 'images has been merged to ' + giffile
    return True, msg


def gif2images(filename, distDir='.', fmt='bmp'):
    """将gif图像每一帧拆成独立的位图 """
    ...
    if fmt == 'bmp' or fmt == 'png':
        mode = 'P'    # image mode 
    im.convert(mode).save(distDir+'/%d.'%cnt+fmt )
    cnt += 1
    try:
        while 1:
            im.seek(im.tell()+1)
    ...
    white = (255, 255, 255)
    preIm = Image.open(distDir+'/%d.' % 0+fmt).convert('RGB')
    size = preIm.size
    prePixs = preIm.load()
    for k in range(1, cnt):
        print '.',
        im = Image.open(distDir+'/%d.' % k+fmt).convert('RGB')
        pixs = im.load()
        ...
        preIm = im
        prePixs = preIm.load()
        im.convert(mode).save(distDir+'/%d.'%k+fmt)
    print '\n', filename, 'has been splited to directory: [', distDir, ']'
    return cnt      # 总帧数


def imgsResize(imgs, size=(800, 600)):
    """使图片大小一致"""
    for img in imgs:
        im = Image.open(img)
...
if __name__ == '__main__':
    ...
    imgPath = r'G:\Workspace\SACCON\20131223SACCON\Pitch\Dynamic\filter\TQData\PNG'
    imgList0 = [os.path.join(imgPath, i) for i in os.listdir(imgPath) if i.startswith('CN') and i.find('AP05') != -1]
    imgsResize(imgList0, size=(2400, 1600))
    gif = images2gif(imgList0,'CN-a.gif',durations=1,loops=0)